...
def fill_process(request):
    if 'product_lists' not in request.session:
        ...
        if form.is_valid():
            ...
            form.save(commit=False)
            form_data = [name, mobile, pincode, address, city, state] # removed pay 
            request.session['form'] = form_data
            return redirect('order:payment_method')
        else:
    ...
    if 'form' not in request.session:
        ...
        if 'product_lists' not in request.session:
            ...
            if request.method == "POST":
                ...
                elif payment=='netbank':

                    request.session['payment'] = "netbank"
                    return render(request, 'order/error_payment.html')

                else:
                    request.session['payment'] = "cre_deb"
                    return render(request,'order/error_payment.html')
            else:
...
def order_summary(request):
    if 'product_lists' not in request.session:
        ...
        for pr, quantitys in zip(products, pr_qun):
            ...
            print('ids-', id)
            try:
                cart_prod = cart.objects.get(cart_auth_id=request.user, check_values=True, cart_product_id=prod)
                cart_prod.check_values = False
                cart_prod.save()
