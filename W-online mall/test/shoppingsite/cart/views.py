...
def link_cart(request, product_name):
    ...
    try:
        cart.objects.get(cart_product_id=products, cart_auth_id=request.user, check_values=True)
        return redirect('cart:view_cart')
    except cart.DoesNotExist:
        cart_prod = cart(cart_product_id=products, cart_auth_id=request.user, check_values=True)
        cart_prod.save()
        return redirect('cart:view_cart')
...
def view_cart(request):
    try:
        cart_prod = cart.objects.filter(cart_auth_id=request.user, check_values=True)
        cart_price = 0.0
        cart_con = 0
...
def plus_minus_quntity(request, product_names):
    ...
    cart_prod = cart.objects.get(cart_auth_id=request.user, cart_product_id=products, check_values=True)
